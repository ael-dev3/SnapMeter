...
export function ActionMix({ snapchain, hypersnap }: { snapchain: SourceMetrics; hypersnap: SourceMetrics }): React.JSX.Element {
  ...
  return (
    <div className="action-mix" role="group" aria-label="Qualifying action mix by source">
      ...
      {FAMILIES.map((family) => {
        ...
        return (
          <div className="mix-row" key={family}>
            ...
            <button type="button" aria-label={`${titleCase(family)}, Snapchain, ${exact(snap)} actions`}>
              <i className="snap-fill" style={{ width: `${(snap / maximum) * 100}%` }} />
              <b>{exact(snap)}</b>
            </button>
            <button type="button" aria-label={`${titleCase(family)}, Hypersnap, ${exact(hyper)} actions`}>
